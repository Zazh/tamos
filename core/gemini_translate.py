...
Главное правило для значений с HTML-разметкой (особенно <span class="hero-fit">, <span class="hero-break">, <span class="text-gold">, <strong>, <em>):
НЕ переводи каждый тег отдельно. Порядок слов в целевом языке отличается от русского, поэтому дословный покусочный перевод даёт бессмыслицу. Действуй в два шага:

  Шаг 1 (перевод). Мысленно собери ВЕСЬ текст значения в одну непрерывную фразу — игнорируя все границы тегов — и переведи её на {target_name} как единое естественное высказывание, с правильным для целевого языка порядком слов.

  Шаг 2 (упаковка обратно в разметку). Возьми перевод из шага 1 и «упакуй» его в ту же структуру тегов, что была в оригинале:
  - Сохрани столько же <span class="hero-fit"> сколько было — это визуальные строки заголовка. Перераспредели перевод на естественные смысловые паузы целевого языка (не по словам RU). Каждая hero-fit-строка — самодостаточный смысловой кусок.
  - <span class="hero-break"> (десктопный перенос) поставь на семантически уместное место в переводе — где на десктопе уместно перенести строку.
  - <span class="text-gold">, <strong>, <em> и прочие inline-акценты поставь на семантически эквивалентные слова в переводе (RU «TOP-100» в text-gold → перевод того же фрагмента в text-gold; для имён/чисел/латиницы перевод обычно совпадает).
  - Если для inline-акцента нет прямого эквивалента в переводе — выбери ближайшее по смыслу слово/фразу.
  - ВАЖНО: между соседними «строчными» спанами (например `</span><span class="hero-fit">` или `</span><span class="hero-break">`) ОБЯЗАТЕЛЬНО ставь пробел — либо в конце предыдущего спана перед `</span>`, либо в начале следующего после `<span ...>`. На десктопе эти спаны становятся `display: inline`, и без пробела слова склеиваются («Школа для детейкоторые» вместо «Школа для детей которые»). Между словами на стыке спанов всегда должен быть ровно один пробел.

Остальные правила:
1. Сохрани все CSS-классы и атрибуты внутри тегов в точности (class="hero-fit", class="text-gold" и т.п.) — НЕ переводи их.
2. Сохрани переносы строк \\n — они смысловые.
3. Тон: дружелюбный, для родителей школьников. Без машинного канцелярита.
4. Длина перевода — близко к оригиналу (±20%); это UI-копия, важно не «растечься».
5. Не добавляй пояснений, кавычек или markdown — только перевод.

Верни строго JSON-объект, где ключи совпадают с input один-в-один, значения — переведённые строки (с восстановленной разметкой).
